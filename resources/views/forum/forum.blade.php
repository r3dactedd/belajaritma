                ...
                <div class="mx-auto rounded-xl bg-white px-4 py-2">
                    <div class="relative m-4">
                        <div class="absolute ml-2 flex h-full items-center">
                            <svg class="text-primary-gray-dark mt-4 h-4 w-4 fill-current" viewBox="0 0 16 16" fill="none"
                        ...
                        <div class="flex">
                            <select
                                ...
                                @foreach ($master as $master)
                                    <option value="for-rent">{{$master->master_type_name}}</option>
                                @endforeach
                    ...
                    <button id="open-btn"
                        class="modal-open my-4 ml-4 flex w-fit items-center rounded-md bg-teal-400 px-4 py-3 text-sm font-semibold text-white transition duration-150 ease-in-out hover:bg-yellow-500 focus:outline-none">
                        Buat Diskusi Baru
                    </button>
                    <div class="mx-auto p-6 antialiased">
                        <div class="space-y-4">
                            ...
                            @foreach ($data as $forum)
                                <a href="forum/{{$forum->id}}" class="flex px-2 hover:bg-gray-200">
                                    ...
                                    <div class="flex-1 rounded-lg px-4 py-2 text-2xl leading-relaxed">
                                        <strong>{{$forum->forum_title}}</strong> <span class="ml-2 text-lg text-gray-400">{{$forum->created_at}}</span>
                                        <div>
                                            <p class="text-sm inline">
                                                Created By {{$forum->formToUser->username}}
                                            </p>
                                            <p class="text-sm inline ml-5">
                                                <b>
                                                    {{$forum->formToMasterType->master_type_name}}
                                                </b>
                                            </p>
                                        </div>
                ...
                <div class="modal-content overflow-y-auto px-2 py-2 text-left md:px-6">
                    <div class="container mx-auto my-5 p-5">
                        ...
                        <div class="mx-auto rounded-xl bg-white px-2 py-8">
                            ...
                            <form action="/createForum" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="mb-4 grid gap-4 sm:mb-5 sm:grid-cols-2 sm:gap-6">
                                    ...
                                    <div class="sm:col-span-2">
                                        ...
                                        <select name="master_type_id" class="w-full rounded-md border-transparent bg-gray-100 px-4 py-3 text-sm font-semibold focus:border-gray-500 focus:bg-white focus:ring-0">
                                            <option value="">Filter berdasarkan Topik</option>
                                            @foreach ($test as $item)
                                                <option value="{{ $item->id }}">{{ $item->master_type_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="sm:col-span-2">
                                        ...
                                        <div id="editorcontainer" class="mt-2 h-32 min-h-full overflow-y-auto">
                                            <div id="editor" style="min-height:60%; height:auto;" value="forum_message"></div>
                                        </div>
                                    </div>
                                </div>
                                <!--Footer-->
                                <div class="mt-8 flex justify-end pt-2">
                                    <button type="submit" class="modal-close mt-2 rounded-lg bg-teal-400 p-3 px-4 text-white hover:bg-indigo-400">Buat Diskusi</button>
                                </div>
                            </form>
                        </div>
